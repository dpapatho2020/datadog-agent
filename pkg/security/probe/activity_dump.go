...
func NewActivityDump(adm *ActivityDumpManager, options ...WithDumpOption) (*ActivityDump, error) {
	...
	if len(ad.OutputDirectory) > 0 {
		...
		ad.outputFile, err = ioutil.TempFile(ad.OutputDirectory, "activity-dump-*."+string(ad.OutputFormat))
		if err != nil {
			return nil, err
		}
		...
		if ad.WithGraph {
			ad.graphFile, err = ioutil.TempFile(ad.OutputDirectory, "graph-dump-*.dot")
			if err != nil {
				ad.close()
				return nil, err
			}
